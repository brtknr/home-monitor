...
endpoint = "{base_url}/{mpan}/meters/{meter_no}/consumption/".format(base_url=base_url,
                                                                     mpan=mpan,
                                                                     meter_no=meter_no)
...
params = dict(page=1)
result = client.query('select * from {measurement} order by time desc limit 1'.format(measurement=measurement))
if result:
